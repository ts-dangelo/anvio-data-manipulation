...
def gen_mcl_input(blastall_results, min_percent_id, minbit_param):
	...
	if len(ids_without_self_search):
		...
		print("blast/diamond did not retun search results for %d of %d the amino acid sequences in your input FASTA file. "
                             "Anvi'o will do some heuristic magic to complete the missing data in the search output to recover "
                             "from this. But since you are a scientist, here are the amino acid sequence IDs for which blast/diamond "
	...
	return (mcl_input_file_path, num_edges_stored, num_minbit_filtered, num_blast_filtered)
