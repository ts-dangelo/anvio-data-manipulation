	...
	for tree in tree_paths:
		...
		for node in t.get_monophyletic(values=[poi], target_attr="Phyla"):
			...
			for sister_node in sisters:
				...
				num_sisters = len(list(set(sister_list)))
				#print(num_sisters)
				sister_len_list.append(num_sisters)
		
		if len(sister_len_list) > 0:
		
			phyla_sister_counts[gc_id] = max(sister_len_list)
			
		else:	
					
			phyla_sister_counts[gc_id] = 0
